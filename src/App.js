...
function App() {
  ...
  const [fmDateDebut, setFmDateDebut] = useState("");
  const [fmDateFin, setFmDateFin] = useState("");
  ...
  const [versementsClientFiltre, setVersementsClientFiltre] = useState("");
  const [versementEdit, setVersementEdit] = useState(null);
  const [versementEditDate, setVersementEditDate] = useState("");
  ...
  const chargerVersementsListe = async () => {
    ...
    try {
      ...
      const filtres = data.filter((v) => { const d = v.date_versement?.substring(0, 10); const okDate = d >= dateDebut && d <= dateFin; const okClient = !versementsClientFiltre || String(v.id_client) === String(versementsClientFiltre); return okDate && okClient; });
  ...
  const enregistrerVersementEdit = async () => {
    if (!versementEditDate || !dateValide(versementEditDate)) { setMessage("Date invalide !"); return; }
    if (!versementEdit.montant || Number(versementEdit.montant) <= 0) { setMessage("Montant invalide !"); return; }
    try {
      const response = await fetch(`${API}/versements/${versementEdit.id_versement}`, { method: "PUT", headers: headers(), body: JSON.stringify({ id_client: versementEdit.id_client, date_versement: parseFR(versementEditDate), montant: versementEdit.montant, mode_paiement: versementEdit.mode_paiement, reference: versementEdit.reference, observation: versementEdit.observation }) });
      const data = await response.json();
      if (data.success) { setMessage("Versement modifie avec succes !"); setVersementEdit(null); setVersementEditDate(""); chargerVersementsListe(); }
...
const imprimerVersementsPDF = () => {
  ...
  const renderVersements = () => (
    <div>
      ...
      {versementEdit && (<div className="modal d-block" style={{ backgroundColor: "rgba(0,0,0,0.5)" }}><div className="modal-dialog"><div className="modal-content"><div className="modal-header bg-warning"><h5 className="modal-title">Modifier Versement</h5><button className="btn-close" onClick={() => setVersementEdit(null)}></button></div><div className="modal-body"><div className="row g-3"><div className="col-md-6"><label className="form-label">Client</label><select className="form-select" value={versementEdit.id_client || ''} onChange={(e) => setVersementEdit({ ...versementEdit, id_client: e.target.value })}>{clients.map((c) => (<option key={c.id_client} value={c.id_client}>{c.code_client} - {c.nom}</option>))}</select></div><div className="col-md-6"><label className="form-label">Date (jj/mm/aaaa)</label><input type="text" className="form-control" maxLength={10} value={versementEditDate} onChange={(e) => setVersementEditDate(e.target.value)} /></div><div className="col-md-6"><label className="form-label">Montant (MRU)</label><input type="number" min="0" className="form-control" value={versementEdit.montant} onChange={(e) => setVersementEdit({ ...versementEdit, montant: e.target.value })} /></div><div className="col-md-6"><label className="form-label">Mode Paiement</label><select className="form-select" value={versementEdit.mode_paiement || ''} onChange={(e) => setVersementEdit({ ...versementEdit, mode_paiement: e.target.value })}><option value="">-- Choisir --</option><option value="Especes">Especes</option><option value="Cheque">Cheque</option><option value="Virement">Virement</option><option value="Autre">Autre</option></select></div><div className="col-md-6"><label className="form-label">Reference</label><input type="text" className="form-control" value={versementEdit.reference || ''} onChange={(e) => setVersementEdit({ ...versementEdit, reference: e.target.value })} /></div><div className="col-md-12"><label className="form-label">Observation</label><input type="text" className="form-control" value={versementEdit.observation || ''} onChange={(e) => setVersementEdit({ ...versementEdit, observation: e.target.value })} /></div></div></div><div className="modal-footer"><button className="btn btn-secondary" onClick={() => setVersementEdit(null)}>Annuler</button><button className="btn btn-warning" onClick={enregistrerVersementEdit}>Enregistrer</button></div></div></div></div>)}
    </div>
  );
  ...
  const [pcDateDebut, setPcDateDebut] = useState("");
  const [pcDateFin, setPcDateFin] = useState("");
    ...
    try {
      ...
      for (const id_produit of produitIds) {
        ...
        const sortiesClient = data.sorties ? data.sorties.filter((s) => {
          ...
          if (!client || s.nom_client !== client.nom) return false;
          const d = s.date_bon ? s.date_bon.substring(0, 10) : "";
          if (pcDateDebut && d < pcDateDebut) return false;
          if (pcDateFin && d > pcDateFin) return false;
          return true;
        }) : [];
...
const imprimerProduitsClientPDF = () => {
    ...
    <div>
      ...
      <div className="card p-3 mb-3 border-primary">
        <div className="row g-3 align-items-end">
          ...
          <div className="col-md-2"><label className="form-label fw-bold">Date debut</label><input type="date" className="form-control" value={pcDateDebut} onChange={(e) => { setPcDateDebut(e.target.value); setPcData(null); }} /></div><div className="col-md-2"><label className="form-label fw-bold">Date fin</label><input type="date" className="form-control" value={pcDateFin} onChange={(e) => { setPcDateFin(e.target.value); setPcData(null); }} /></div><div className="col-md-2"><button className="btn btn-primary w-100" onClick={chargerProduitsClient} disabled={!pcClientSelectionne}>🔍 Afficher</button>
          </div>
        </div>
      </div>
  ...
  const construireTableauMouvements = () => {
    ...
    mouvements.forEach((m) => {
      if (m._type === "entree") { stockCourant += Number(m.quantite); lignesMouvements.push({ date: formatDateFR(m.date_bon.substring(0, 10)), numero_bon: m.numero_bon, _dateISO: m.date_bon.substring(0, 10), type: "Entree", tiers: m.nom_fournisseur, entree: Number(m.quantite), sortie: "-", stock: stockCourant, _classe: "table-success" }); }
      else { stockCourant -= Number(m.quantite); lignesMouvements.push({ date: formatDateFR(m.date_bon.substring(0, 10)), numero_bon: m.numero_bon, _dateISO: m.date_bon.substring(0, 10), type: "Sortie", tiers: m.nom_client, entree: "-", sortie: Number(m.quantite), stock: stockCourant, _classe: "table-danger" }); }
    });
    if (fmDateDebut || fmDateFin) { lignesMouvements = lignesMouvements.filter((l) => { const d = l._dateISO || ""; if (fmDateDebut && d < fmDateDebut) return false; if (fmDateFin && d > fmDateFin) return false; return true; }); }
      ...
      {ficheStockData && !loadingFicheStock && (
        <div className="card p-4">
          ...
          <table className="table table-bordered table-striped table-hover">
            <thead className="table-dark"><tr><th>Code</th><th>Designation</th><th>Unite</th><th className="text-end text-info">Stock Initial</th><th className="text-end text-success">Total Entrees</th><th className="text-end text-danger">Total Sorties</th><th className="text-end text-primary fw-bold">Stock Disponible</th></tr></thead>
            <tbody>{ficheStockData.lignes.map((l, i) => (<tr key={i} className={Number(l.stock_disponible) <= 0 ? "table-danger" : ""}><td>{l.code_produit}</td><td>{l.designation}</td><td>{l.unite}</td><td className="text-end">{Number(l.stock_initial).toLocaleString("fr-FR")}</td><td className="text-end text-success fw-bold">+{Number(l.total_entrees).toLocaleString("fr-FR")}</td><td className="text-end text-danger fw-bold">-{Number(l.total_sorties).toLocaleString("fr-FR")}</td><td className={`text-end fw-bold ${Number(l.stock_disponible) <= 0 ? "text-danger" : "text-primary"}`}>{Number(l.stock_disponible).toLocaleString("fr-FR")}</td></tr>))}</tbody>
          </table>
        </div>
      )}
    </div>
    ...
    <div>
      ...
      <div className="card p-3 mb-4">
        <div className="row g-2 align-items-end">
          <div className="col-md-4"><label className="form-label fw-bold">Choisir un Produit</label><select className="form-select" value={produitSelectionne} onChange={(e) => { setProduitSelectionne(e.target.value); setFicheMouvements(null); }}><option value="">-- Selectionner un produit --</option>{produits.map((p) => (<option key={p.id_produit} value={p.id_produit}>{p.code_produit} — {p.designation}</option>))}</select></div>
          <div className="col-md-2"><label className="form-label fw-bold">Date debut</label><input type="date" className="form-control" value={fmDateDebut} onChange={(e) => setFmDateDebut(e.target.value)} /></div><div className="col-md-2"><label className="form-label fw-bold">Date fin</label><input type="date" className="form-control" value={fmDateFin} onChange={(e) => setFmDateFin(e.target.value)} /></div><div className="col-md-3"><button className="btn btn-primary w-100" onClick={chargerMouvements} disabled={!produitSelectionne}>🔍 Afficher les Mouvements</button></div>
        </div>
      </div>
        ...
        return (
          <div className="card p-4">
            ...
            <table className="table table-bordered table-hover">
              ...
              <tbody>{tableauLignes.map((ligne, i) => (<tr key={i} className={ligne._classe}><td>{ligne.date}</td><td>{ligne.numero_bon}</td><td>{ligne.type === "Stock Initial" && <span className="badge bg-info text-dark">📦 Stock Initial</span>}{ligne.type === "Entree" && <span className="badge bg-success">⬆️ Entree</span>}{ligne.type === "Sortie" && <span className="badge bg-danger">⬇️ Sortie</span>}</td><td>{ligne.tiers}</td><td className="text-end fw-bold text-success">{ligne.entree !== "-" ? Number(ligne.entree).toLocaleString("fr-FR") : ""}</td><td className="text-end fw-bold text-danger">{ligne.sortie !== "-" ? Number(ligne.sortie).toLocaleString("fr-FR") : ""}</td><td className="text-end fw-bold text-primary">{Number(ligne.stock).toLocaleString("fr-FR")}</td></tr>))}</tbody>
              <tfoot className="table-dark fw-bold"><tr><td colSpan="4" className="text-end">TOTAUX :</td><td className="text-end text-success">{Number(totaux.total_entrees).toLocaleString("fr-FR")}</td><td className="text-end text-danger">{Number(totaux.total_sorties).toLocaleString("fr-FR")}</td><td className="text-end text-warning">{Number(totaux.stock_final).toLocaleString("fr-FR")}</td></tr></tfoot>
            </table>
            <div className="row mt-3">
              <div className="col-md-3"><div className="card text-white bg-info text-center p-2"><small>Stock Initial</small><h4>{Number(totaux.qte_initiale).toLocaleString("fr-FR")}</h4></div></div>
              <div className="col-md-3"><div className="card text-white bg-success text-center p-2"><small>Total Entrees</small><h4>+{Number(totaux.total_entrees).toLocaleString("fr-FR")}</h4></div></div>
              <div className="col-md-3"><div className="card text-white bg-danger text-center p-2"><small>Total Sorties</small><h4>-{Number(totaux.total_sorties).toLocaleString("fr-FR")}</h4></div></div>
              <div className="col-md-3"><div className="card text-white bg-primary text-center p-2"><small>Stock Final</small><h4>{Number(totaux.stock_final).toLocaleString("fr-FR")}</h4></div></div>
            </div>
          </div>
        );
      })()}
  ...
  const imprimerSituationFinancierePDF = () => {
    ...
    doc.setFillColor(220, 53, 69); doc.rect(15, finalY, 60, 12, "F");
    doc.setFillColor(25, 135, 84); doc.rect(80, finalY, 60, 12, "F");
    doc.setFillColor(13, 110, 253); doc.rect(145, finalY, 65, 12, "F");
    doc.setFillColor(255, 140, 0); doc.rect(215, finalY, 67, 12, "F");
    doc.setTextColor(255, 255, 255);
    doc.text(`Solde Initial : ${fmt(totaux.solde_initial)} MRU`, 45, finalY + 8, { align: "center" });
    doc.text(`Total S.V : ${fmt(totaux.total_valeur_sv)} MRU`, 110, finalY + 8, { align: "center" });
    doc.text(`TOTAL CREANCE : ${fmt(totaux.total_creance)} MRU`, 177, finalY + 8, { align: "center" });
    doc.text(`CREANCE NETTE : ${fmt(totaux.creance_nette)} MRU`, 248, finalY + 8, { align: "center" });
    const pageHeight = doc.internal.pageSize.height;
    doc.setFontSize(8); doc.setTextColor(150, 150, 150); doc.setFont("helvetica", "normal");
      ...
      : bonDetail ? (
        <div className="card p-4">
          ...
          <table className="table table-bordered table-striped"><thead className="table-dark"><tr><th>Code</th><th>Designation</th><th>Quantite</th><th>Prix Unitaire</th><th>Montant</th></tr></thead>
            <tbody>{lignesDetail.map((l, i) => (<tr key={i}><td>{l.code_produit}</td><td>{l.designation}</td><td className="text-end">{Number(l.quantite).toLocaleString("fr-FR")}</td><td className="text-end">{Number(l.prix_unitaire).toLocaleString("fr-FR")}</td><td className="text-end">{Number(l.montant).toLocaleString("fr-FR")} MRU</td></tr>))}</tbody>
            <tfoot className="table-secondary fw-bold"><tr><td colSpan="4" className="text-end">TOTAL GENERAL :</td><td className="text-end">{lignesDetail.reduce((sum, l) => sum + Number(l.montant || 0), 0).toLocaleString("fr-FR")} MRU</td></tr></tfoot>
          </table>
        </div>
      ) : (
        <>
        ...
        {page === "graphiques" ? renderGraphiques()
          ...
          : (
            <>
              ...
              {loading ? (<div className="text-center"><div className="spinner-border text-primary"></div></div>) : (
                <table className="table table-bordered table-striped table-hover">
                  <thead className="table-dark"><tr>{colonnes[page] && colonnes[page].map((col) => (<th key={col} className={["stock_initial","total_entree","total_sortie","stock_actuel","prix_achat","prix_vente","stock_minimum"].includes(col) ? "text-end" : ""}>{col.replace(/_/g, " ").toUpperCase()}</th>))}{["produits", "clients", "fournisseurs"].includes(page) && <th>ACTIONS</th>}</tr></thead>
                  <tbody>{donneesFiltrees.map((d, i) => (<tr key={i}>{colonnes[page] && colonnes[page].map((col) => { const colsNumeriques = ["stock_initial","total_entree","total_sortie","stock_actuel","prix_achat","prix_vente","stock_minimum"]; const isNum = colsNumeriques.includes(col); return (<td key={col} className={isNum ? "text-end fw-bold" : ""}>{isNum && d[col] !== null && d[col] !== undefined ? Number(d[col]).toLocaleString("fr-FR") : d[col]}</td>); })}{["produits", "clients", "fournisseurs"].includes(page) && (<td className="text-center">{isAdmin && <button className="btn btn-warning btn-sm me-2" onClick={() => ouvrirModification(d)}>✏️ Modifier</button>}{isAdmin && <button className="btn btn-danger btn-sm" onClick={() => supprimerElement(d[idCols[page]])}>🗑️ Supprimer</button>}{!isAdmin && <span className="text-muted small">Consultation seulement</span>}</td>)}</tr>))}</tbody>
                </table>
              )}
            </>
          )}
