...
function App() {
  ...
  const enregistrerVersementEdit = async () => {
    ...
    try {
      const response = await fetch(`${API}/versements/${versementEdit.id_versement}`, { method: "PUT", headers: headers(), body: JSON.stringify({ date_versement: parseFR(versementEditDate), montant: versementEdit.montant, mode_paiement: versementEdit.mode_paiement, reference: versementEdit.reference, observation: versementEdit.observation }) });
      const data = await response.json();
